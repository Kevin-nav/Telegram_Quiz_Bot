...
# Tanjah Theme - Iteration 10 (Minimalist Apple-inspired)
...
\node[inner sep=0pt] (content) {
    ...
    % Question Box
    % Minimalist: clean white background, thicker left border for accent
    \begin{tcolorbox}[enhanced, colback=white, colframe=mainGreen!30!white, 
        ...
        fonttitle=\large\bfseries\sffamily, title=Question, coltitle=mainGreen,
        colbacktitle=white, attach boxed title to top left={yshift=-2.5mm, xshift=3mm},
...
% Scaled down to 4.5 to fit nicely inside the bounds, keeping the stylish font
\node[overlay, rotate=45, scale=4.5, opacity=0.06, color=mainGreen] at (content.center) {\fontfamily{pzc}\selectfont\Huge TANJAH};
\end{tikzpicture}
\end{document}"""

if __name__ == "__main__":
    output_img = "test_images/iteration_10_minimalist_apple.png"
    print(f"Attempting to render iteration 10 LaTeX to {output_img}...")
    ...
    # Thinner borders, accent color on the left edge, white backgrounds
