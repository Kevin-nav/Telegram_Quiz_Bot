...
class StudentCourseStateRepository:
    ...
    async def get(self, user_id: int, course_id: str) -> StudentCourseState | None:
        async with self.session_factory() as session:
            return await self._get_state(session, user_id, course_id)

    async def get_or_create(self, user_id: int, course_id: str) -> StudentCourseState:
        async with self.session_factory() as session:
            state = await self._get_state(session, user_id, course_id)
            if state is None:
    ...
    async def update_fields(
        self, user_id: int, course_id: str, **updates
    ...
    async def _get_state(
        self, session, user_id: int, course_id: str
    ) -> StudentCourseState | None:
        result = await session.execute(
            select(StudentCourseState).where(
